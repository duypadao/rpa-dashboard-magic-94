import { Translations } from "../LanguageProvider";

export const translations: Translations = {
    common: {
        dashboard: {
            en: "RPA Dashboard",
            cn: "RPA 仪表板",
            vi: "RPA Trung tâm"
        },
        overviewTab: {
            en: "Overview",
            cn: "概览",
            vi: "Tổng quan"
        },
        aiInsightsTab: {
            en: "AI Insights",
            cn: "AI 洞察",
            vi: "AI Insights"
        },
        status: {
            en: "Status",
            cn: "状态",
            vi: "Trạng thái",
        },
        lastRun: {
            en: "Last Run",
            cn: "上次运行",
            vi: "Gần nhất"
        },
        duration: {
            en: "Duration",
            cn: "持续时间",
            vi: "Thời gian"
        },
        details: {
            en: "Details",
            cn: "详情",
            vi: "Chi tiết"
        },
        processFlow: {
            en: "Process Flow",
            cn: "流程图",
            vi: "Quy trình"
        },
        defaultProcessFlow: {
            en: "Default Process Flow",
            cn: "默认流程图",
            vi: "Quy trình mặc định"
        },
        notifications: {
            en: "Notifications",
            cn: "通知",
            vi: "Thông báo"
        },
        myAccount: {
            en: "My Account",
            cn: "我的账户",
            vi: "Tài khoản của tôi"
        },
        profile: {
            en: "Profile",
            cn: "个人资料",
            vi: "Cá nhân"
        },
        settings: {
            en: "Settings",
            cn: "设置",
            vi: "Cài đặt",
        },
        logout: {
            en: "Log out",
            cn: "退出登录",
            vi: "Đăng xuất",
        },
        robotError: {
            en: "RPA Robot Error",
            cn: "RPA 机器人错误",
            vi: "Robot Lỗi"
        },
        invoiceRobotError: {
            en: "Invoice Processing Robot #1 has encountered an error",
            cn: "发票处理机器人 #1 遇到错误",
            vi: "invoiceRobotError"
        },
        timeAgo: {
            en: "min ago",
            cn: "分钟前",
            vi: "phút trước"
        },
        language: {
            en: "Language",
            cn: "语言",
            vi: "Ngôn ngữ"
        },
        result: {
            en: "Result",
            cn: "结果",
            vi: "Kết quả"
        },
        success: {
            en: "Success",
            cn: "成功",
            vi: "Thành công"
        },
        warning: {
            en: "Warning",
            cn: "警告",
            vi: "Cảnh báo"
        },
        failure: {
            en: "Failure",
            cn: "失败",
            vi: "Thất bại"
        },
        backToDashboard: {
            en: "Back to Dashboard",
            cn: "返回仪表板",
            vi: "Quay lại"
        },
        inform: {
            en: "Inform",
            cn: "通知",
            vi: "Nhắc nhở"
        },
        notificationSent: {
            en: "Notification sent",
            cn: "通知已发送",
            vi: "Đã gửi thông báo"
        },
        notificationSentDesc: {
            en: "A notification about",
            cn: "关于",
            vi: "Thông báo"
        },
        notificationSentDescEnd: {
            en: "has been sent to the team",
            cn: "的通知已发送给团队",
            vi: "đã gửi cho nhóm"
        },
        history: {
            en: "Run History",
            cn: "运行历史",
            vi: "Lịch sử"
        },
        analytics: {
            en: "AI Analytics",
            cn: "AI 分析",
            vi: "AI Phân tích"
        },
        robotStatusOverview: {
            en: "Robot Status Overview",
            cn: "机器人状态概览",
            vi: "Trạng thái Robot"
        },
        monitorAllRobots: {
            en: "Monitor and manage all automation robots from a single dashboard",
            cn: "从单个仪表板监控和管理所有自动化机器人",
            vi: "Quản lý toàn bộ robot"
        },
        searchRobots: {
            en: "Search robots...",
            cn: "搜索机器人...",
            vi: "Tìm kiếm robot..."
        },
        filterByStatus: {
            en: "Filter by status",
            cn: "按状态筛选",
            vi: "Lọc theo trạng thái"
        },
        allRobots: {
            en: "All Robots",
            cn: "所有机器人",
            vi: "Tất cả"
        },
        running: {
            en: "Running",
            cn: "运行中",
            vi: "Đang chạy"
        },
        stopped: {
            en: "Stopped",
            cn: "已停止",
            vi: "Tạm dừng"
        },
        error: {
            en: "Error",
            cn: "错误",
            vi: "Lỗi"
        },
        aiInsights: {
            en: "AI Insights",
            cn: "AI 洞察",
            vi: "AI Insights"
        },
        noInsightsAvailable: {
            en: "No insights available at this time",
            cn: "目前没有可用的洞察",
            vi: "Không có gì"
        },
        robot: {
            en: "Robot",
            cn: "机器人",
            vi: "Robot"
        },
        couldNotLoadRobots: {
            en: "Could not load robots",
            cn: "无法加载机器人",
            vi: "Không thể tải thông tin"
        },
        tryAgainLater: {
            en: "Please try again later",
            cn: "请稍后再试",
            vi: "Vui lòng thử lại"
        },
        robotsOverview: {
            en: "Robots Overview",
            cn: "机器人概览",
            vi: "Tổng quan Robot"
        },
        availableInsights: {
            en: "Available Insights",
            cn: "可用洞察",
            vi: "Phân tích chuyên sâu"
        },
        currentTask: {
            en: "Current Task",
            cn: "currentTask",
            vi: "Công việc"
        },
        runningRobot: {
            en: "Running Robot",
            cn: "runningRobot",
            vi: "Robot đang làm việc"
        },
        limsNo: {
            en: "LIMS No",
            cn: "LIMS",
            vi: "Mã đơn LIMS"
        },
        automationType: {
            en: "Automation Type",
            cn: "automationType",
            vi: "Loại hình"
        },
        activeRobots: {
            en: "Active Robots",
            cn: "activeRobots",
            vi: "Robot trực tuyến"
        },
        successTasks: {
            en: "Success Tasks",
            cn: "successTasks",
            vi: "Số công việc thành công"
        },
        totalTasks: {
            en: "Total tasks",
            cn: "totalTasks",
            vi: "Tổng số công việc"
        },
        errorRate: {
            en: "Error Rate",
            cn: "errorRate",
            vi: "Tỉ lệ lỗi"
        },
        savingTime: {
            en: "Saving Time",
            cn: "savingTime",
            vi: "Tiết kiệm thời gian"
        },
        robotRunningTime: {
            en: "Robot Running Time",
            cn: "robotRunningTime",
            vi: "Thời gian robot chạy"
        },
        "robotWork {{rate}} % thanManualWork": {
            en: "Robot work {{rate}} % faster than manual work ",
            cn: "robotWork {{rate}} % thanManualWork",
            vi: "Robot làm việc nhanh hơn {{rate}}%  "
        },
        labRobotForUploadingReport: {
            en: "Upload report automatically",
            cn: "labRobotForUploadingReport",
            vi: "Tải báo cáo tự động"
        },
        LABRobots: {
            en: "Labratory Robot",
            cn: "LABRobots",
            vi: "Robot Phòng Thí Nghiệm"
        },
        "{{seconds}}s": {
            en: "{{seconds}}s",
            cn: "{{seconds}}s",
            vi: "{{seconds}} giây"
        },
        "{{minutes}}m{{seconds}}s": {
            en: "{{minutes}}m{{seconds}}s",
            cn: "{{minutes}}m{{seconds}}s",
            vi: "{{minutes}}phút {{seconds}} giây"
        },
        "just now": {
            en: "just now",
            cn: "just now",
            vi: "Vừa xong"
        },
        "totalRobot": {
            en: "Total Robot",
            cn: "totalRobot",
            vi: "Tổng số Robot"
        },
        "{{diffSecs}} seconds ago": {
            en: "{{diffSecs}} seconds ago",
            cn: "{{diffSecs}} seconds ago",
            vi: "{{diffSecs}} giây trước"
        },
        "{{diffMins}} minutes ago": {
            en: "{{diffMins}} minutes ago",
            cn: "{{diffMins}} minutes ago",
            vi: "{{diffMins}} phút trước"
        },
        "{{diffMins}} minute ago": {
            en: "{{diffMins}} minute ago",
            cn: "{{diffMins}} minute ago",
            vi: "{{diffMins}} phút trước"
        },
        lastRunTime: {
            en: "Last Run Time",
            cn: "上次运行时间",
            vi: "Thời gian chạy lần cuối"
        },
        overview: {
            en: "Overview",
            cn: "概览",
            vi: "Tổng quan"
        },
        start: {
            en: "Start",
            vi: "Bắt đầu",
            cn: "开始"
        },
        end: {
            en: "End",
            vi: "Kết thúc",
            cn: "结束"
        },
        all: {
            en: "All",
            cn: "全部",
            vi: "Tất cả"
        },
        idle: {
            en: "Idle",
            cn: "空闲",
            vi: "Nhàn rỗi"
        },
        viewPdf: {
            en: "View PDF",
            cn: "查看 PDF",
            vi: "Xem PDF"
        },
        actions: {
            en: "Action",
            cn: "操作",
            vi: "Thao tác"
        },
        noDetailsAvailable: {
            en: "No details available",
            cn: "无可用详情",
            vi: "Không có chi tiết"
        },
        Id: {
            en: "ID",
            cn: "编号",
            vi: "STT"
        },
        timeSaved: {
            en: "Time Saved",
            cn: "节省时间",
            vi: "Thời gian tiết kiệm"
        },
        accumulate: {
            en: "Accumulated",
            cn: "累计",
            vi: "tích luỹ"
        },
        total: {
            en: "Total",
            cn: "总计",
            vi: "tổng"
        },
        filterByMonth: {
            en: "Filter by Month",
            cn: "按月筛选",
            vi: "Lọc theo tháng"
        },
        month: {
            en: "Month",
            cn: "月份",
            vi: "Tháng"
        },
        year: {
            en: "Year",
            cn: "年份",
            vi: "Năm"
        },
        clear: {
            en: "Clear",
            cn: "清除",
            vi: "Xoá"
        },
        apply: {
            en: "Apply",
            cn: "应用",
            vi: "Áp dụng"
        },

        detail: {
            en: "Details",
            cn: "详情",
            vi: "Chi tiết"
        },
        type: {
            en: "Type",
            cn: "类型",
            vi: "Loại"
        },
        "{{hours}}h{{seconds}}s":{
            en: "{{hours}}h{{seconds}}s",
            cn: "{{hours}}h{{seconds}}s",
            vi: "{{hours}}giờ {{seconds}}giây",
        },
        "{{diffHours}} hours ago": {
            en: "{{diffHours}} hours ago",
            cn: "{{diffHours}} hours ago",
            vi: "{{diffHours}} giờ trước"
        },
        "{{diffHours}} hour ago": {
            en: "{{diffHours}} hour ago",
            cn: "{{diffHours}} hour ago",
            vi: "{{diffHours}} giờ trước"
        },
        "{{hours}}h{{minutes}}m": {
            en: "{{hours}}h {{minutes}}m",
            cn: "{{hours}}h{{minutes}}m",
            vi: "{{hours}} giờ {{minutes}} giây"
        },
        "{{hours}}h{{minutes}}m{{seconds}}s": {
            en: "{{hours}}h {{minutes}}m {{seconds}}s",
            cn: "{{hours}}h{{minutes}}m",
            vi: "{{hours}} giờ {{minutes}} phút {{seconds}} giây"
        },
        "{{minutes}}m": {
            en: "{{minutes}}m",
            cn: "{{minutes}}m",
            vi: "{{minutes}} phút"
        },
        robotPerformance: {
            en: "Robot Performance",
            cn: "robotPerformance",
            vi: "Hiệu suất làm việc của robot"
        },
        LIMS_BRAReportPrintAndUpload: {
            en: "Bra Washing Report",
            cn: "LIMS_BRAReportPrintAndUpload",
            vi: "Báo cáo giặt áo"
        },
        LIMS_UploadOldReportAndReturnSample: {
            en: "Upload report then return sample",
            cn: "LIMS_UploadOldReportAndReturnSample",
            vi: "Tải báo cáo và trả mẫu"
        },
        labRobotReport: {
            en: "LAB Report",
            cn: "labRobotReport",
            vi: "Báo cáo LAB"
        },
        LABRobotReport: {
            en: "Labratory Robot Report",
            cn: "LABRobotReport",
            vi: "Báo cáo robot phòng thí nghiệm"
        },
        summaryLABRobotPerformance: {
            en: "Summrize labratory robot performance",
            cn: "summaryLABRobotPerformance",
            vi: "Tổng hợp kết quả robot phòng thí nghiệm"
        },
        today: {
            en: "Today",
            cn: "today",
            vi: "Hôm nay"
        },
        thisWeek: {
            en: "This week",
            cn: "thisWeek",
            vi: "Tuần này"
        },
        thisMonth: {
            en: "This month",
            cn: "thisMonth",
            vi: "Tháng này"
        },
        thisYear: {
            en: "This year",
            cn: "thisYear",
            vi: "Năm nay"
        },
        customRange: {
            en: "Custom range",
            cn: "customRange",
            vi: "Tùy chọn"
        },
        startDate: {
            en: "Start Date",
            cn: "startDate",
            vi: "Từ ngày"
        },
        endDate: {
            en: "End Date",
            cn: "endDate",
            vi: "Tới ngày"
        },
        LIMS_UploadReportWithConclusion: {
            en: "Report with conclusion",
            cn: "LIMS_UploadReportWithConclusion",
            vi: "Báo cáo có lời bình"
        },
        LIMS_LABCLogoStamp: {
            en: "Logo and Stamp",
            cn: "LIMS_LABCLogoStamp",
            vi: "Logo và tem"
        },
        LIMS_LABCAccessory: {
            en: "Accessory",
            cn: "LIMS_LABCAccessory",
            vi: "Phụ liệu"
        },
        totalRobots: {
            en: "Total Robots",
            cn: "totalRobots",
            vi: " Tổng số Robot"
        },
        successRate: {
            en: "Success Rate",
            cn: "successRate",
            vi: "Tỉ lệ thành công"
        },
        errorTasks: {
            en: "Error Tasks",
            cn: "successRate",
            vi: "Số công việc bị lỗi"
        },
        labRobotChartTitle: {
            en: "Analyze Robot Tasks",
            cn: "labRobotChartTitle",
            vi: "Phân tích công việc của từng robot"
        }
    },
    invoice: {
        invoiceOverView: {
            en: "Invoice Overview",
            cn: "发票概览",
            vi: "Tổng quan hoá đơn"
        },
        invoiceProcessingHistory: {
            en: "Invoice Processing History",
            cn: "发票处理历史",
            vi: "Lịch sử xử lý hoá đơn"
        },
        invoiceProcessingAnalytics: {
            en: "Invoice Processing Analytics",
            cn: "发票处理分析",
            vi: "Phân tích xử lý hoá đơn"
        },
        invoiceCreated: {
            en: "Invoices Created",
            cn: "已创建发票",
            vi: "Hoá đơn đã tạo"
        },
        warningInvoice: {
            en: "Warning Invoices",
            cn: "警告发票",
            vi: "Hoá đơn cảnh báo"
        },
        failureInvoice: {
            en: "Failed Invoices",
            cn: "失败发票",
            vi: "Hoá đơn thất bại"
        },
        successRate: {
            en: "Success Rate",
            cn: "成功率",
            vi: "Tỷ lệ thành công"
        },
        searchSupplier: {
            en: "Search supplier...",
            cn: "搜索供应商...",
            vi: "Tìm nhà cung cấp..."
        },
        supplierId: {
            en: "Supplier ID",
            cn: "供应商编号",
            vi: "Mã nhà cung cấp"
        },
        supplierName: {
            en: "Supplier Name",
            cn: "供应商名称",
            vi: "Tên nhà cung cấp"
        },
        inlineDate: {
            en: "Go-live Date",
            cn: "上线日期",
            vi: "Ngày triển khai"
        },
        invoiceInQueue: {
            en: "In Queue",
            cn: "队列中",
            vi: "Trong hàng đợi"
        },
        invoiceNo: {
            en: "Invoice No.",
            cn: "发票号",
            vi: "Số hoá đơn"
        },
        invoiceRunDate: {
            en: "Run Date",
            cn: "运行日期",
            vi: "Ngày chạy"
        },
        invoiceCheckProcess: {
            en: "Check Process",
            cn: "检查流程",
            vi: "Kiểm tra xử lý"
        },
        searchInvoiceOrSupplier: {
            en: "Search invoice or supplier...",
            cn: "搜索发票或供应商...",
            vi: "Tìm hoá đơn hoặc nhà cung cấp..."
        },
        processFlowForInvoice: {
            en: "Invoice Process Flow",
            cn: "发票处理流程",
            vi: "Luồng xử lý hoá đơn"
        },
        invoiceProcessDescription: {
            en: "Detailed visualization of the invoice processing steps and their statuses.",
            cn: "发票处理步骤及其状态的详细可视化。",
            vi: "Trực quan chi tiết các bước xử lý hoá đơn và trạng thái của chúng."
        },
    },
    mspo: {
        rundays: {
            en: "Run Days",
            cn: "运行天数",
            vi: "Ngày chạy"
        },
        runDaysWithPO: {
            en: "Run Days with PO",
            cn: "采购订单运行天数",
            vi: "Ngày chạy có PO"
        },
        totalDownloadedPOs: {
            en: "Total Downloaded POs",
            cn: "已下载采购订单总数",
            vi: "Tổng số PO đã tải"
        },
        mspoSummary: {
            en: "MSPO Summary",
            cn: "MSPO 总结",
            vi: "Tổng hợp MSPO"
        },
        mspoDetails: {
            en: "MSPO Details",
            cn: "MSPO 详情",
            vi: "Chi tiết MSPO"
        },
        mspoRunDate: {
            en: "Run Date",
            cn: "运行日期",
            vi: "Ngày chạy"
        },
        selectFromSummary: {
            en: "Select an item from the summary to view details",
            cn: "从摘要中选择一个项目以查看详情",
            vi: "Chọn một mục từ bảng tổng hợp để xem chi tiết"
        },
        mspoProcessingAnalytics: {
            en: "MSPO Processing Analytics",
            cn: "MSPO 处理分析",
            vi: "Phân tích xử lý MSPO"
        },

    },




};